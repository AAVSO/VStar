...
public class PhaseCoordSource implements ICoordSource {
	...
	/**
	 * Twice the number of items in the map, since we want to
	 * facilitate a Standard Phase Diagram where the phase ranges
	 * over -1..1 inclusive.
	 * 
	...
	public double getXCoord(int series, int item,
		...
		if (item < itemCount) {
			...
			// 0..1
			phase = seriesNumToObSrcListMap.get(series).get(item % itemCount).getStandardPhase();			
		}
	...
	/**
	 * The actual item number for the Y coordinate is modulo the number 
	 * of elements in the series in this case.
	 * 
	 * @param series The series of interest. 
	 * @param item The target item.
	 * @param seriesNumToObSrcListMap A mapping from series number to a list of observations.
