/**
 * VStar: a statistical analysis tool for variable star data.
 * Copyright (C) 2016  AAVSO (http://www.aavso.org/)
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>. 
 */
package org.aavso.tools.vstar.util.date;

/**
 * Solar coordinates.
 */
public class SolarCoords {

	private double ra;
	private double dec;
	private double M;
	private double C;
	
	/**
	 * Constructor
	 * 
	 * @param ra The Sun's RA in decimal degrees.
	 * @param dec The Sun's declination in decimal degrees.
	 * @param M The true solar anomaly.
	 * @param C The Sun's equation of center.
	 */
	public SolarCoords(double ra, double dec, double M, double C) {
		super();
		this.ra = ra;
		this.dec = dec;
		this.M = M;
		this.C = C;
	}

	/**
	 * @return the Sun's right ascension in decimal degrees.
	 */
	public double getRA() {
		return ra;
	}

	/**
	 * @return the Sun's declination in decimal degrees.
	 */
	public double getDec() {
		return dec;
	}

	/**
	 * @return the Sun's RA in decimal degrees.
	 */
	public double getRa() {
		return ra;
	}

	/**
	 * @return the m
	 */
	public double getTrueAnomaly() {
		return M;
	}

	/**
	 * @return the Sun's equation of center.
	 */
	public double getEquationOfCenter() {
		return C;
	}
}
