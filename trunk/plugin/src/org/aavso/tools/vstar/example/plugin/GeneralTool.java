...
public class GeneralTool extends GeneralToolPluginBase {
	...
	private List<ITextComponent<String>> aidSeries() {
		...
		List<ITextComponent<String>> components = new ArrayList<ITextComponent<String>>();

		createAIDSeriesComponents(seriesList, components);
		
		return components;
	}

	private void createAIDSeriesComponents(List<SeriesType> seriesList,
			List<ITextComponent<String>> seriesComponentList) {

		for (SeriesType series : seriesList) {
			seriesComponentList.add(new TextField(series.getDescription(),
					series.getShortName(), true, false));
		}
	}
}
