...
// #any comment
// <JD|HJD><delim><MAG>[<delim><MAG_ERR>[<delim><ODSCODE>[<delim>[<VALIDATION_FLAG>[<delim><FILTER>[<delim><any additional fields are ignored>]]]]]]
//
...
public class ExtendedTextFileFormatObservationSource extends
	...
	class ExtendedTextFileFormatRetriever extends AbstractObservationRetriever {
			...
			try {
				...
				for (String line : lines) {
					...
					try {
							...
							if (!isEmpty(line)) {
								if (line.startsWith("#")) {
									...
									if (errorText != null) {
										...
										throw new ObservationValidationError("Reading terminated due to fatal error: " + errorText);
									}
								} else {
									//String[] fields = line.split(String.valueOf(delimiter));
									String[] fields = splitWithQuotes(line, delimiter, DEFAULT_QUOTEMARK, escapingQuotes);
									ValidObservation ob = readNextObservation(fields, lineNum);
						...
						if (error == null && isEmpty(error)) error = e.toString();
		...
		private Pair<String, String> translateDelimiter(String delim) {
			...
			if ("tab".equalsIgnoreCase(delim)) {
				...
				if (ordVal < 32 || ordVal > 126) {
					return new Pair<String, String>(null, String.format("Ordinal delimiter value '%d' out of range 32..126", ordVal));
				}
		...
		private ValidObservation readNextObservation(String[] fields, int lineNum) 
				throws ObservationValidationError, ObservationValidationWarning {
			
			String observationWarnings = null;
			
			if (timeColumn < 0 || magColumn < 0 || fields.length <= timeColumn || fields.length <= magColumn) {
				throw new ObservationValidationError("At least two fields expected: Time and Magnitude");
			}
			...
			if (magnitude.isBrighterThan()) {
				throw new ObservationValidationError("Was '>' intended (brighter than) or '<'?"); // See CommonTextFormatValidator.java
			}
				...
				if (!isEmpty(uncertaintyStr)) 
				{
					try	{
						...
						if (!ignoreValidationErrors) {
							...
							if (observationWarnings != null) observationWarnings += "; "; else observationWarnings = "";
			...
			// observation.setMagnitude must be set AFTER assignment of uncertainty (because of caching!)
		 ...
		 *             quote (char)
		 * @param  escapingQuote
		 *             if true, use \" for quotes inside quoted field, otherwise use ""
		 * @return the array of strings computed by splitting the input
		 *         
		 */
		private String[] splitWithQuotes(String s, char delim, char quote, boolean escapingQuote)
		{
			...
			while (pos < len) {
				if (inQuotes) {
					if (pos == len-1) {
						...
						if (s.charAt(pos) == (escapingQuote ? esc : quote) && s.charAt(pos+1) == quote) {
							field.append(quote);
							pos++;				
							pos++;
