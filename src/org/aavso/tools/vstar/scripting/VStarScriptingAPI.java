...
public class VStarScriptingAPI {
	...
	if (obSinkPlugin != null) {
	    ...
	    MessageBox.showErrorDialog("Observation Save",
		    String.format("%s observation sink plug-in not found", LocaleProps.get("TEXT_FORMAT_FILE")));
	}
	...
	for (ObservationSinkPluginBase plugin : PluginLoader.getObservationSinkPlugins()) {
	    if (pluginName.contains(plugin.getDisplayName())) {
		obSinkPlugin = plugin;
		break;
	    }
	}
